...
- **逆运动学模式** — 3D 拖拽目标球，实时 IK 驱动机构跟随运动
...
单页应用，无构建工具。FastAPI 后端用于本地开发调试。

| 模块 | 职责 |
|------|------|
| `app/server.py` | FastAPI 开发服务器 |
...
| `app/static/mode-manager.js` | 设计 / 网格 / IK 三模式切换 |
| `app/static/grid-mode.js` | β₁×β₂ 热力图渲染 |
| `app/static/workspace.js` | U 工作空间曲面可视化 |
| `app/static/ik-mode.js` | IK 拖拽交互 |
