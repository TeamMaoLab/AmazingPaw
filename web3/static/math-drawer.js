...
const SECTIONS = [
  ...
  },
  {
    ...
    blocks: [
      ...
      { type: 'p', text: '关节 1 是唯一的全局旋转（绕 AB，∥X），使手指子系统倾斜。关节 2-5 都是局部 Y 方向旋转，手指内部运动始终在局部 XZ 平面内。系统 = 全局倾斜 θ + 平面内四连杆 ∠KAP。' },
...
<svg viewBox="0 0 460 310" width="100%" style="font-family:'SF Mono',Menlo,monospace;font-size:11px;">
  ...
  },
  {
    ...
    blocks: [
      ...
      { type: 'math', latex: '\\mathbf{q}_{passive} = (\\theta,\\ \\angle KAP)^T' },
      { type: 'ul', items: [
        'θ — APB 绕 AB 方向的旋转角（关节 1，轴承在 B 处）',
        '∠KAP — 关节 A 处 A→K 与 A→P 两方向之间的夹角',
      ]},
      { type: 'p', text: '∠KAP 是四连杆 P-A-K-Q-P 的被动变量，唯一确定其构型。' },
      { type: 'p', html: true, text: '工作变量：κ = 180° − ∠KAP，即 A→K 方向从 +X 计量的方向角。初始值 ∠KAP₀ = 180° − (α+γ) = 75°，κ₀ = α+γ = 105°。' },
      { type: 'p', html: true, text: '运动中：κ = α + γ + Δ（Δ 为偏离初始值的变化量），T 方向有效角度 α\' = κ − γ = α + Δ。' },
      { type: 'h3', text: '4.3 两阶段求解' },
      { type: 'p', html: true, text: '阶段 1 — 驱动系统：给定 (β₁, β₂)，由连杆长度约束求解被动变量 (θ, ∠KAP)。' },
      { type: 'math', latex: '\\begin{cases} f_1(\\theta,\\ \\angle KAP;\\ \\beta_1) = 0 \\\\ f_2(\\theta,\\ \\angle KAP;\\ \\beta_2) = 0 \\end{cases}' },
      { type: 'p', html: true, text: '阶段 2 — 结构四连杆：给定 (θ, ∠KAP)，计算 κ = 180° − ∠KAP，各点位置解析计算。Q 由 KQ 圆与 PQ 圆交点确定（解析，无需迭代）。' },
    ],
  },
  {
    title: '5. 约束方程',
    blocks: [
      ...
      { type: 'p', text: 'APB 上 B、P、A 均在 AB 轴上，Rx(θ) 不改变其坐标。下游点受 θ 和 ∠KAP 影响。' },
      { type: 'p', html: true, text: '记 κ = 180° − ∠KAP（A→K 方向角），α\' = κ − γ（T 方向的有效角度）。K、T、R、L 的全局坐标：' },
      { type: 'math', latex: 'K(\\theta, \\kappa) = A + R_x(\\theta) \\cdot \\begin{pmatrix} L_{AK}\\cos\\kappa \\\\ 0 \\\\ L_{AK}\\sin\\kappa \\end{pmatrix}' },
      { type: 'math', latex: 'T(\\theta, \\kappa) = A + R_x(\\theta) \\cdot \\begin{pmatrix} L_{AT}\\cos(\\kappa{-}\\gamma) \\\\ 0 \\\\ L_{AT}\\sin(\\kappa{-}\\gamma) \\end{pmatrix}' },
      { type: 'math', latex: 'R = T + R_x(\\theta)\\cdot(0,\\,\\text{BarHalf},\\,0), \\quad L = T + R_x(\\theta)\\cdot(0,\\,-\\text{BarHalf},\\,0)' },
      { type: 'p', html: true, text: 'Q 的位置：K 已知后，Q 由 KQ 圆（半径 L<sub>QK</sub>）与 PQ 圆（半径 L<sub>PQ</sub>）的交点解析确定（至多 2 解，由装配构型选唯一分支）。' },
      ...
      { type: 'math', latex: '\\boxed{f_1(\\theta, \\angle KAP;\\ \\beta_1) = \\|R(\\theta, \\kappa) - D(\\beta_1)\\|^2 - L_{rod\\_r}^2 = 0}' },
      { type: 'math', latex: '\\boxed{f_2(\\theta, \\angle KAP;\\ \\beta_2) = \\|L(\\theta, \\kappa) - F(\\beta_2)\\|^2 - L_{rod\\_l}^2 = 0}' },

      { type: 'h3', text: '5.4 约束方程展开' },
      { type: 'p', html: true, text: '记 κ = 180° − ∠KAP，α\' = κ − γ（T 方向的有效角度）。' },
      ...
      { type: 'p', html: true, text: '这四个约束由各刚体内部几何自动满足。∠KAP 唯一确定四连杆构型：给定 ∠KAP，由 κ = 180° − ∠KAP 直接计算 K，再由 KQ 圆与 PQ 圆交点解析确定 Q。' },
    ],
  },
  {
    title: '6. 求解流程',
    blocks: [
      { type: 'h3', text: '6.1 Newton-Raphson 求解 (θ, κ)' },
      { type: 'p', html: true, text: '内部以工作变量 κ = 180° − ∠KAP 求解。' },
      { type: 'math', latex: 'J = \\begin{bmatrix} \\dfrac{\\partial f_1}{\\partial \\theta} & \\dfrac{\\partial f_1}{\\partial \\kappa} \\\\[6pt] \\dfrac{\\partial f_2}{\\partial \\theta} & \\dfrac{\\partial f_2}{\\partial \\kappa} \\end{bmatrix}' },
      { type: 'math', latex: '\\begin{pmatrix} \\theta \\\\ \\kappa \\end{pmatrix}^{(k+1)} = \\begin{pmatrix} \\theta \\\\ \\kappa \\end{pmatrix}^{(k)} - J^{-1} \\begin{pmatrix} f_1 \\\\ f_2 \\end{pmatrix}^{(k)}' },
      { type: 'p', html: true, text: '注意 ∂f/∂(∠KAP) = −∂f/∂κ（符号反转）。迭代收敛后，∠KAP = 180° − κ。' },

      { type: 'h3', text: '6.2 Jacobian 解析表达式' },
      { type: 'p', html: true, text: '记 α\' = κ − γ。' },
      ...
      { type: 'p', text: 'f₁ 对 κ 的偏导：' },
      { type: 'math', latex: '\\frac{\\partial f_1}{\\partial \\kappa} = 2L_{AT}\\left[-\\Delta x_1 \\sin\\alpha\' - \\Delta y_1 \\cos\\alpha\' \\sin\\theta + \\Delta z_1 \\cos\\alpha\' \\cos\\theta\\right]' },
      ...
      { type: 'h3', text: '6.3 计算顺序' },
      {
        ...
        rows: [
          ...
          ['5', '$\\theta, \\angle KAP$', 'Newton-Raphson（内部变量 κ = 180° − ∠KAP）', '$\\beta_1, \\beta_2$'],
          ['6', '$K$', '$A + R_x(\\theta) \\cdot (L_{AK}\\cos\\kappa,\\,0,\\,L_{AK}\\sin\\kappa)$', '$\\theta, \\kappa$'],
          ['7', '$Q$', 'KQ 圆与 PQ 圆交点解析', '$\\theta, \\kappa$'],
          ['8', '$U$', 'UQK 三角形', '$Q, K$'],
          ['9', '$T$', '$A + R_x(\\theta) \\cdot (L_{AT}\\cos(\\kappa{-}\\gamma),\\,0,\\,L_{AT}\\sin(\\kappa{-}\\gamma))$', '$\\theta, \\kappa$'],
  ...
  },
  {
    title: '7. 自由度分析',
    blocks: [
      { type: 'p', html: true, text: '广义坐标 q = (θ, ∠KAP, β₁, β₂)，共 4 个变量。' },
      ...
      { type: 'p', html: true, text: '四连杆有 1 个内部自由度。θ 和 ∠KAP 是独立的被动变量。' },
    ],
  },
];
